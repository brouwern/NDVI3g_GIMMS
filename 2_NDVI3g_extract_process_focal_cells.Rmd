...
NDVI.MVC.mo <- dcast(data = dat3,
                     formula = yr + mo.num + site + mo.num3 ~ mo.ab,
                     value.var = "NDVI",
                     fun.aggregate = max)
```
...
Examine differences
```{r}
plot(b ~ a, data = NDVI.MVC.mo)
```

## Calculate monthly maximum

```{r}
NDVI.MVC.mo$NDVI.MVC.mo <- apply(NDVI.MVC.mo[,c("a","b")],
                                 1, max)
```
...
```{r}
mo. <- ifelse(as.numeric(NDVI.MVC.mo$mo.num) > 9, 
              NDVI.MVC.mo$mo.num ,
                paste(0,NDVI.MVC.mo$mo.num, sep = ""))
...
```{r}
NDVI.MVC.mo$mo.num<- as.numeric(NDVI.MVC.mo$mo.num)

i.win <- which(NDVI.MVC.mo$mo.num %in% c(1,2,3))#1st dry season
i.spr <- which(NDVI.MVC.mo$mo.num %in% c(4,5,6))#1st rain season
i.sum <- which(NDVI.MVC.mo$mo.num %in% c(7,8,9))#2nd dry season
i.fal <- which(NDVI.MVC.mo$mo.num %in% c(10,11,12))#2nd rainy season
```
